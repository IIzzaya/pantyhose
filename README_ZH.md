...
基于 [txthinking/socks5](https://github.com/txthinking/socks5) 构建，支持 TCP (CONNECT) 和 UDP (UDP ASSOCIATE)，可选 username/password 认证。TLS SNI remap 默认开启，IPv6 默认禁用。
...
**典型场景**：公司 Windows 电脑通过专线可以直接访问外网（YouTube、Google 等）。个人 macOS/Linux 电脑通过 VPN（如飞连 CorpLink）连接公司内网，但 DNS 被污染。Pantyhose 将两者打通——个人电脑的全部流量通过公司电脑的网络出口转发。
...
2. **Proxy Settings**：在菜单栏 **Proxy > Proxy Settings...** 中，将 **Proxy IP/Domain** 设置为运行 pantyhose 的内网 IP（如 `10.154.38.77`），**Proxy Port** 设置为 pantyhose 监听端口（默认 `1080`）。
...
**提示**：在客户端禁用 IPv6 可以进一步减少 DNS 污染问题：
